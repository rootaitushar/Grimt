ALTER TABLE public.placement_students
  ADD COLUMN is_lateral_entry BOOLEAN NOT NULL DEFAULT false;
...
ALTER TABLE public.placement_students
  ADD CONSTRAINT placement_students_semester_statuses_valid CHECK (
    (
      (
        is_lateral_entry
        AND semester_1_status IS NULL AND semester_1_marks IS NULL AND semester_1_reappears = 0
        AND semester_2_status IS NULL AND semester_2_marks IS NULL AND semester_2_reappears = 0
      )
      OR (
        NOT is_lateral_entry
        AND semester_1_status IS NOT NULL AND semester_1_status IN ('Clear', 'Backlog', 'Result Awaited')
    ...
    AND (
      (semester_5_status IS NULL AND semester_5_marks IS NULL AND semester_5_reappears = 0)
      OR (
        semester_5_status IN ('Clear', 'Backlog', 'Result Awaited')
